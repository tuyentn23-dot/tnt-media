# build_gacha_viral.py - video gacha girl viral
...
# Voice
VO = os.path.abspath('output/gacha_vo.mp3')
...
fps = 8
# FIX: render du frame (+0.5s) de ffmpeg cat theo -shortest
n = int((DUR + 0.5) * fps)
n_img = len(imgs)
seg = DUR / n_img
covers = [fit(Image.open(p).convert('RGB'), BW, BH) for p in imgs]
tmp = 'output/_gacha_frames'
...
# Caption
cap_tmp = 'output/_gacha_caps'
os.makedirs(cap_tmp, exist_ok=True)
dts = []
for k, (a, b) in enumerate(segs):
	fp = cap_tmp + chr(47) + ('c%d.txt' % k)
	words = PARTS[k].split()
	lines = []
	cur = ''
	for w in words:
	...
	s += ':fontsize=60:fontcolor=0xFFFFFF:borderw=6:bordercolor=0xFF1493:shadowx=4:shadowy=4:shadowcolor=0x000000@0.9:line_spacing=12'
	s += ':x=(w-text_w)/2:y=h-380'
	dts.append(s)
vf = ','.join(dts)
pat = os.path.join(tmp, 'f%04d.png')
silent = 'output/gacha_silent.mp4'
...
final = os.path.abspath('output/Mialinhcute_gacha_viral.mp4')
cmd2 = [FFMPEG, '-y', '-i', silent, '-i', VO, '-i', music, '-filter_complex', '[1:a]volume=1.6[a1];[2:a]volume=0.2[a2];[a1][a2]amix=inputs=2:duration=first[aout]', '-map', '0:v', '-map', '[aout]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-shortest', final]
