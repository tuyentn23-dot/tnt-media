# build_mia_kids.py - builder kenh Mia kids (9:16, caption to ro)
...
def build(vid):
	...
	pack_dir = "output/mia_" + vid
	out = os.path.abspath("output/Mialinhcute_kids_" + vid + ".mp4")
	print("=== building", vid)
	vo = os.path.abspath("output/mia_vo_" + vid + ".mp3")
	...
	tmp = "output/kids" + vid + "_frames"
	os.makedirs(tmp, exist_ok=True)
	for i in range(n):
		t = i / fps
		idx = min(n_img - 1, int(t / seg))
		local_t = (t % seg) / seg
		zoom = 1.0 + 0.18 * local_t
		base = covers[idx]
		nw, nh = int(BW * zoom), int(BH * zoom)
		im2 = base.resize((nw, nh), Image.LANCZOS)
		px = int(25 * math.sin(t * 2.5))
		py = int(18 * math.cos(t * 2.0))
	...
	for k, (a, b) in enumerate(segs):
		...
		s += ":fontsize=64:fontcolor=0xFFFFFF:borderw=7:bordercolor=0xFF1493:shadowx=5:shadowy=5:shadowcolor=0x000000@0.95:line_spacing=14"
		s += ":x=(w-text_w)/2:y=h-380"
		dts.append(s)
	vf = ",".join(dts)
	pat = os.path.join(tmp, "f%04d.png")
	silent = "output/kids" + vid + "_silent.mp4"
	...
	cmd2 = [FFMPEG, "-y", "-i", silent, "-i", vo, "-i", music, "-filter_complex", "[1:a]volume=1.7[a1];[2:a]volume=0.2[a2];[a1][a2]amix=inputs=2:duration=first[aout]", "-map", "0:v", "-map", "[aout]", "-t", str(DUR), "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", out]
	r2 = subprocess.run(cmd2, capture_output=True, text=True)
	print("mux rc:", r2.returncode)
	print("final:", os.path.getsize(out) if os.path.exists(out) else 0)
	return out
if __name__ == "__main__":
	vid = sys.argv[1] if len(sys.argv) > 1 else "pikachu_facts"
	result = build(vid)
	print("RESULT:", result)